...
#define ERROR(message, ...) spdlog::error("At [{}]-[{}]-[{}]: {}", __FILE__, __FUNCTION__, __LINE__, message, __VA_ARGS__); std::exit(0);
#define WARN(message, ...) spdlog::warn("At [{}]-[{}]-[{}]: {}", __FILE__, __FUNCTION__, __LINE__, message, __VA_ARGS__)
#define DEBUG(message, ...) spdlog::debug("At [{}]-[{}]-[{}]: {}", __FILE__, __FUNCTION__, __LINE__, message, __VA_ARGS__)
...
#define ERROR_IF(statement, message) if (statement) { \
    spdlog::error("At [{}]-[{}]-[{}]: {}", __FILE__, __FUNCTION__, __LINE__, message); \
    std::exit(0); \
}

#define WARN_IF(statement, message, ...) if (statement) { \
    spdlog::warn("At [{}]-[{}]-[{}]: {}", __FILE__, __FUNCTION__, __LINE__, message, __VA_ARGS__); \
}
